...
import os
import shutil									# used for deleting whole directory
import zipfile									# for zipping all files in a folder
...
def page_input():
   ...
   try:
      ...
      probs = soup.find('table', class_="problems").find_all('tr')
      to_return['problems'] = []
      for i in range(1, len(probs)):
         curr_prob = {}
         curr_prob['id'] = probs[i].a.string.strip()
         curr_prob['name'] = probs[i].div.a.string.strip()
         curr_prob['link'] = "https://codeforces.com" + str(probs[i].a.get('href'))
...
def download_problems():
   ...
   for prob_indx in download_list:
      prob_id = to_return['problems'][int(prob_indx)]['id']
      prob_name = to_return['problems'][int(prob_indx)]['name']
      pdfName = f"{prob_id}. {prob_name}.pdf"
      ...
      pdfkit.from_string(html, os.path.join(folder, pdfName), options=options, configuration=config)
